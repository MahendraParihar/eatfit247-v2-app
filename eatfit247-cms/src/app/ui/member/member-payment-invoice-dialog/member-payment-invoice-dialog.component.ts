...
import { IAddress, IMemberPayment, IResponse } from 'shared-lib';
...
export class MemberPaymentInvoiceDialogComponent implements OnInit {
  ...
  dialogData: any;
  memberPaymentObj: IMemberPayment;
  ...
  async downloadInvoice(): Promise<void> {
    const res = await this.httpService.getRequest<IResponse<any>>(ApiUrlEnum.MEMBER_PAYMENT_INVOICE_DOWNLOAD, this.id, null, true);
    if (res) {
      if (res.data) {
  ...
  async sendInvoice(): Promise<void> {
    const res = await this.httpService.getRequest(ApiUrlEnum.MEMBER_PAYMENT_INVOICE_SEND, this.id, null, true);
    if (res) {
      this.snackBarService.showSuccess('Invoice sent successfully');
    }
  }

  private async loadDataById(): Promise<void> {
    const res: any = await this.httpService.getRequest<IResponse<IMemberPayment>>(ApiUrlEnum.MEMBER_PAYMENT_MANAGE, this.id, null, true);
    if (res) {
