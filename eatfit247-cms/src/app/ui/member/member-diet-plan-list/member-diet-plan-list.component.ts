...
import { DietPlanStatusEnum, DietTypeEnum, IDropdownItem, IResponse } from 'shared-lib';
import {
...
import { MemberDietPlanDatasource } from 'src/app/ui/member/member-diet-plan.datasource';
...
export class MemberDietPlanListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    this.dataSource.dietTemplate.subscribe((count: IDropdownItem[]) => this.dietTemplateList = count);
  ...
  async downloadDietPlan(dietPlanDetail: CyclePlan, index: number): Promise<void> {
    let url;
