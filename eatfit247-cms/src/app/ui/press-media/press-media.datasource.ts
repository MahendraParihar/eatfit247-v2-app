...
export class PressMediaDatasource implements DataSource<PressMediaModel> {
  private dataSubject = new BehaviorSubject<PressMediaModel[]>([]);
  ...
  connect(collectionViewer: CollectionViewer): Observable<PressMediaModel[]> {
  ...
  async loadData(url: ApiUrlEnum, payload: any): Promise<boolean> {
    ...
    switch (apiResponse.code) {
      case ServerResponseEnum.SUCCESS:
        ...
        const tempList: PressMediaModel[] = [];
        for (let s of apiResponse.data.list) {
          tempList.push(PressMediaModel.fromJson(s));
        }
        this.dataSubject.next(tempList);
        return true;
      case ServerResponseEnum.WARNING:
        const tempPressMediaList: PressMediaModel[] = [];
        this.totalCountSubject.next(tempPressMediaList.length);
        this.dataSubject.next(tempPressMediaList);
