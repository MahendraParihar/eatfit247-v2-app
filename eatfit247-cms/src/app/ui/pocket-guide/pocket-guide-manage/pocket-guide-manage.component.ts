...
import { FileTypeEnum, IManagePocketGuide, IResponse } from 'shared-lib';
...
export class PocketGuideManageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  lovModelObj: IManagePocketGuide;
  id: number;
  ...
  async loadDataById(id: number): Promise<void> {
    const res = await this.httpService.getRequest<IResponse<IManagePocketGuide>>(ApiUrlEnum.POCKET_GUIDE_MANAGE, id, null, true);
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.id > 0) {
      await this.httpService.putRequest(ApiUrlEnum.POCKET_GUIDE_MANAGE, this.id, payload, true);
    } else {
      await this.httpService.postRequest(ApiUrlEnum.POCKET_GUIDE_MANAGE, payload, true);
    }
