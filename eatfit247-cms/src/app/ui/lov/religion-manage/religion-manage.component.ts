...
export class ReligionManageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataById(id: number): Promise<void> {
    const res = await this.httpService.getRequest<IResponse<ILov>>(ApiUrlEnum.RELIGION_MANAGE, id, null, true);
    if (res) {
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.id > 0) {
      await this.httpService.putRequest(ApiUrlEnum.RELIGION_MANAGE, this.id, payload, true);
    } else {
      await this.httpService.postRequest(ApiUrlEnum.RELIGION_MANAGE, payload, true);
    }
