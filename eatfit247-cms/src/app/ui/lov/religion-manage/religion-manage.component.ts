...
export class ReligionManageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataById(id: number): Promise<void> {
    const res: ResponseDataModel = await this.httpService.getRequest(ApiUrlEnum.RELIGION_MANAGE, id, null, true);
    if (res) {
      switch (res.code) {
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.id > 0) {
      res = await this.httpService.putRequest(ApiUrlEnum.RELIGION_MANAGE, this.id, payload, true);
    } else {
      res = await this.httpService.postRequest(ApiUrlEnum.RELIGION_MANAGE, payload, true);
    }
    if (res) {
      switch (res.code) {
        case ServerResponseEnum.SUCCESS:
