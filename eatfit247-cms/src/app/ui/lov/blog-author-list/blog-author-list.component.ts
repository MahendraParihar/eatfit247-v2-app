...
import { IBlogAuthor, ITableListFilter } from 'shared-lib';
...
export class BlogAuthorListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  dataSource: TableDataDatasource<IBlogAuthor>;
  totalCount = 0;
  ...
  onDeleteClick(item: IBlogAuthor, index: number) {
  ...
  async updateStatusTask(item: IBlogAuthor, index: number): Promise<void> {
    const payload = {
      active: !item.active
    };
    const res = await this.httpService.patchRequest(ApiUrlEnum.BLOG_AUTHOR_STATUS_CHANGE, item.id, payload, true);
    if (res){
