...
export class BloodSugarManageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataById(id: number): Promise<void> {
    const res = await this.httpService.getRequest<IResponse<ILov>>(ApiUrlEnum.BLOOD_SUGAR_MANAGE, id, null, true);
    if (res) {
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.id > 0) {
      await this.httpService.putRequest(ApiUrlEnum.BLOOD_SUGAR_MANAGE, this.id, payload, true);
    } else {
      await this.httpService.postRequest(ApiUrlEnum.BLOOD_SUGAR_MANAGE, payload, true);
    }
