...
export class MaritalStatusListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataSet(): Promise<void> {
    ...
    await this.dataSource.loadData(ApiUrlEnum.MARITAL_STATUS_LIST, this.payload);
  }
  ...
  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.MARITAL_STATUS_MANAGE, id);
  }
  ...
  async updateStatusTask(item: LovModel, index: number): Promise<void> {
    ...
    const res: ResponseDataModel = await this.httpService.patchRequest(ApiUrlEnum.MARITAL_STATUS_STATUS_CHANGE, item.id, payload, true);
    if (res) {
      switch (res.code) {
