...
export class ProgramCategoryManageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataById(id: number): Promise<void> {
    const res: ResponseDataModel = await this.httpService.getRequest(ApiUrlEnum.PROGRAM_CATEGORY_MANAGE, id, null, true);
    if (res) {
      switch (res.code) {
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.id > 0) {
      res = await this.httpService.putRequest(ApiUrlEnum.PROGRAM_CATEGORY_MANAGE, this.id, payload, true);
    } else {
      res = await this.httpService.postRequest(ApiUrlEnum.PROGRAM_CATEGORY_MANAGE, payload, true);
    }
    if (res) {
      switch (res.code) {
        case ServerResponseEnum.SUCCESS:
