...
export class TypeOfExerciseListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataSet(): Promise<void> {
    ...
    await this.dataSource.loadData(ApiUrlEnum.TYPE_OF_EXERCISE_LIST, this.payload);
  }
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.TYPE_OF_EXERCISE_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.TYPE_OF_EXERCISE_MANAGE, id);
  }
  ...
  async updateStatusTask(item: LovModel, index: number): Promise<void> {
    ...
    const res: ResponseDataModel = await this.httpService.patchRequest(ApiUrlEnum.TYPE_OF_EXERCISE_STATUS_CHANGE, item.id, payload, true);
    if (res) {
      switch (res.code) {
