...
import { IBlogCategory, ILov, ITableListFilter } from 'shared-lib';
@Component({
  standalone: false,
...
export class CallPurposeListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataSet(): Promise<void> {
    ...
    await this.dataSource.loadData(ApiUrlEnum.CALL_PURPOSE_LIST, this.payload);
  }

  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.CALL_PURPOSE_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.CALL_PURPOSE_MANAGE, id);
  }
  ...
  async updateStatusTask(item: ILov, index: number): Promise<void> {
    ...
    const res = await this.httpService.patchRequest(ApiUrlEnum.CALL_PURPOSE_STATUS_CHANGE, item.id, payload, true);
    if (res){
