...
@Component({
  selector: 'app-sleeping-pattern-list',
  templateUrl: './sleeping-pattern-list.component.html',
  styleUrls: ['./sleeping-pattern-list.component.scss'],
})
export class SleepingPatternListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataSet(): Promise<void> {
    ...
    await this.dataSource.loadData(ApiUrlEnum.SLEEPING_PATTERN_LIST, this.payload);
  }
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.SLEEPING_PATTERN_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.SLEEPING_PATTERN_MANAGE, id);
  }
  ...
  async updateStatusTask(item: LovModel, index: number): Promise<void> {
    ...
    const res: ResponseDataModel = await this.httpService.patchRequest(ApiUrlEnum.SLEEPING_PATTERN_STATUS_CHANGE, item.id, payload, true);
    if (res) {
      switch (res.code) {
