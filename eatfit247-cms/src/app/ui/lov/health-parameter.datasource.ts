...
export class HealthParameterDatasource implements DataSource<HealthParameterModel> {

  private dataSubject = new BehaviorSubject<HealthParameterModel[]>([]);
  ...
  connect(collectionViewer: CollectionViewer): Observable<HealthParameterModel[]> {
  ...
  async loadData(url: ApiUrlEnum, payload: any): Promise<boolean> {
    ...
    switch (apiResponse.code) {
      case ServerResponseEnum.SUCCESS:
        ...
        const tempList: HealthParameterModel[] = [];
        for (let s of apiResponse.data.list) {
          tempList.push(HealthParameterModel.fromJson(s));
        }
