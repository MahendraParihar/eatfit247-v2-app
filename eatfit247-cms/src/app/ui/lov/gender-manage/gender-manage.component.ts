...
export class GenderManageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataById(id: number): Promise<void> {
    const res = await this.httpService.getRequest<IResponse<ILov>>(ApiUrlEnum.GENDER_MANAGE, id, null, true);
    if (res) {
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.id > 0) {
      await this.httpService.putRequest(ApiUrlEnum.GENDER_MANAGE, this.id, payload, true);
    } else {
      await this.httpService.postRequest(ApiUrlEnum.GENDER_MANAGE, payload, true);
    }
