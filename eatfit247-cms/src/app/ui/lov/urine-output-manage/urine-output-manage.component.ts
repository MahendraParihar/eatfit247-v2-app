...
export class UrineOutputManageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataById(id: number): Promise<void> {
    const res: ResponseDataModel = await this.httpService.getRequest(ApiUrlEnum.URINE_OUTPUT_MANAGE, id, null, true);
    if (res) {
      switch (res.code) {
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.id > 0) {
      res = await this.httpService.putRequest(ApiUrlEnum.URINE_OUTPUT_MANAGE, this.id, payload, true);
    } else {
      res = await this.httpService.postRequest(ApiUrlEnum.URINE_OUTPUT_MANAGE, payload, true);
    }
    if (res) {
      switch (res.code) {
        case ServerResponseEnum.SUCCESS:
