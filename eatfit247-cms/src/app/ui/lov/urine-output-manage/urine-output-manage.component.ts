...
export class UrineOutputManageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataById(id: number): Promise<void> {
    const res = await this.httpService.getRequest<IResponse<ILov>>(ApiUrlEnum.URINE_OUTPUT_MANAGE, id, null, true);
    if (res) {
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.id > 0) {
      await this.httpService.putRequest(ApiUrlEnum.URINE_OUTPUT_MANAGE, this.id, payload, true);
    } else {
      await this.httpService.postRequest(ApiUrlEnum.URINE_OUTPUT_MANAGE, payload, true);
    }
