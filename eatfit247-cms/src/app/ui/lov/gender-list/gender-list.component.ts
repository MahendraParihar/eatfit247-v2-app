...
export class GenderListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataSet(): Promise<void> {
    ...
    await this.dataSource.loadData(ApiUrlEnum.GENDER_LIST, this.payload);
  }
  ...
  async updateStatusTask(item: LovModel, index: number): Promise<void> {
    ...
    const res: ResponseDataModel = await this.httpService.patchRequest(ApiUrlEnum.GENDER_STATUS_CHANGE, item.id, payload, true);
    if (res) {
      switch (res.code) {
