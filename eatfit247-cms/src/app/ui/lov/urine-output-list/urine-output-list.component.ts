...
export class UrineOutputListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataSet(): Promise<void> {
    ...
    await this.dataSource.loadData(ApiUrlEnum.URINE_OUTPUT_LIST, this.payload);
  }
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.URINE_OUTPUT_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.URINE_OUTPUT_MANAGE, id);
  }
  ...
  async updateStatusTask(item: LovModel, index: number): Promise<void> {
    ...
    const res: ResponseDataModel = await this.httpService.patchRequest(ApiUrlEnum.URINE_OUTPUT_STATUS_CHANGE, item.id, payload, true);
    if (res) {
      switch (res.code) {
