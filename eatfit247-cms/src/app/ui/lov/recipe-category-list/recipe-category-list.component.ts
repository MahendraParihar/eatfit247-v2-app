...
import { IRecipeCategory, ITableListFilter } from 'shared-lib';
@Component({
  standalone: false,
...
export class RecipeCategoryListComponent implements OnInit, AfterViewInit, OnDestroy {
  displayedColumns = ['seqNo', 'title', 'fromTime', 'toTime', 'status', 'createdBy', 'createdAt', 'updatedBy', 'updatedAt', 'action'];
  dataSource: TableDataDatasource<IRecipeCategory>;
  totalCount = 0;
  ...
  onAddClick() {
    this.navigationService.navigateTo(NavigationPathEnum.RECIPE_CATEGORY_MANAGE);
  }

  onEditClick(id: number) {
    this.navigationService.navigateToById(NavigationPathEnum.RECIPE_CATEGORY_MANAGE, id);
  }

  onDeleteClick(item: IRecipeCategory, index: number) {
  ...
  async updateStatusTask(item: IRecipeCategory, index: number): Promise<void> {
    const payload = {
      active: !item.active
    };
    const res = await this.httpService.patchRequest(ApiUrlEnum.RECIPE_CATEGORY_STATUS_CHANGE, item.id, payload, true);
