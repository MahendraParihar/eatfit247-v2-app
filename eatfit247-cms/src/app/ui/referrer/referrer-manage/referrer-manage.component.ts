...
import { IDropdownItem, FileTypeEnum, IReferrer, IResponse } from 'shared-lib';
...
export class ReferrerManageComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async loadDataById(id: number): Promise<void> {
    const res = await this.httpService.getRequest<IResponse<IReferrer>>(ApiUrlEnum.REFERRER_MANAGE, id, null, true);
    if (res && res.data) {
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.id > 0) {
      await this.httpService.putRequest<IResponse<void>>(ApiUrlEnum.REFERRER_MANAGE, this.id, payload, true);
    } else {
      await this.httpService.postRequest<IResponse<void>>(ApiUrlEnum.REFERRER_MANAGE, payload, true);
    }
  ...
  async loadMetaData(): Promise<void> {
    this.franchiseList = [];
    const res = await this.httpService.getRequest<IResponse<IDropdownItem[]>>(ApiUrlEnum.FRANCHISE_DD_LIST, null, null, true);
    if (res && res.data) {
