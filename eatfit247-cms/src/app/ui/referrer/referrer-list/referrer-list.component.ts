...
import { IReferrer, ITableListFilter } from 'shared-lib';
...
export class ReferrerListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  dataSource: TableDataDatasource<IReferrer>;
  totalCount = 0;
  ...
  onDeleteClick(item: IReferrer, index: number) {
  ...
  async updateStatusTask(item: IReferrer, index: number): Promise<void> {
    const payload = {
      active: !item.active
    };
    const res = await this.httpService.patchRequest(ApiUrlEnum.REFERRER_STATUS_CHANGE, item.id, payload, true);
    if (res) {
