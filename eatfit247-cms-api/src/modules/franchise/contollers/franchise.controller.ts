...
export class FranchiseController {
  ...
  @UseGuards(JwtAuthGuard)
  @Post('manage')
  async create(@Req() req, @Body() body: CreateFranchiseDto) {
    return await this.service.create(body, req.ip, req.user.userId);
  }

  @UseGuards(JwtAuthGuard)
  @Put('manage/:id')
  async update(@Param('id') id: number, @Req() req: any, @Body() body: CreateFranchiseDto) {
  ...
  @UseGuards(JwtAuthGuard)
  @Get('list-franchise')
  async listFranchise(@Req() req: any) {
    return await this.service.findRoleBasedFranchise(req.user.userId);
  }
}
