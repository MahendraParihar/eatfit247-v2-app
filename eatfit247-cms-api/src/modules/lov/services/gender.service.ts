...
export class GenderService {
  ...
  public async findAll(searchDto: IBasicSearch): Promise<ITableList<ILov>> {
    ...
    const { rows, count } = await this.genderRepository.findAndCountAll<MstGender>({
      include: [
        {
          model: MstAdminUser,
          required: false,
  ...
  private async createInDB(obj: any) {
    return await this.genderRepository.create(obj);
  }

  private async updateInDB(id: number, obj: any) {
    return await this.genderRepository.update(obj, { where: { genderId: id } });
  }
}
