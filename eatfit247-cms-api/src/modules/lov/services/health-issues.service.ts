...
export class HealthIssuesService {
  constructor(
    @InjectModel(MstHealthIssues) private readonly healthIssuesRepository: typeof MstHealthIssues,
  ) {}
  ...
  public async findAll(searchDto: IBasicSearch): Promise<ITableList<ILov>> {
    ...
    const { rows, count } = await this.healthIssuesRepository.findAndCountAll<MstHealthIssues>({
      include: [
        {
          model: MstAdminUser,
          required: false,
  ...
  public async fetchById(id: number): Promise<ILov> {
    const find = await this.healthIssuesRepository.findOne({
      where: {
        healthIssueId: id,
      },
    });
    ...
    return <ILov>{
      id: find.healthIssueId,
      name: find.healthIssue,
      active: find.active,
  ...
  private async createInDB(obj: any) {
    return await this.healthIssuesRepository.create(obj);
  }

  private async updateInDB(id: number, obj: any) {
    return await this.healthIssuesRepository.update(obj, { where: { healthIssueId: id } });
  }
}
