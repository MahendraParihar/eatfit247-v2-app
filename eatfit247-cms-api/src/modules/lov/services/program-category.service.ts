...
export class ProgramCategoryService {
  ...
  public async findAll(searchDto: BasicSearchDto): Promise<IServerResponse> {
    ...
    try {
      ...
      const { rows, count } = await this.programCategoryRepository.findAndCountAll<MstProgramCategory>({
        include: [
          {
            model: MstAdminUser,
            required: false,
      ...
      for (const s of rows) {
        const iEvent: ILov = {
          id: s.programCategoryId,
          name: s.programCategory,
          active: s.active,
  ...
  public async fetchById(id: number): Promise<IServerResponse> {
    let res: IServerResponse;
    try {
      const find = await this.programCategoryRepository.findOne({
        where: {
      ...
      if (find) {
        const dataObj = <ILov>{
          id: find.programCategoryId,
          name: find.programCategory,
          active: find.active,
  ...
  public async getProgramCategoryList(): Promise<DropdownListInterface[]> {
    const tempList = await this.programCategoryRepository.findAll<MstProgramCategory>({
      where: {
        active: true,
      },
      order: [['programCategory', 'ASC']],
  ...
  private async createInDB(obj: any) {
    return await this.programCategoryRepository
      .create(obj)
      .then((result) => {
        return result;
      })
  ...
  private async updateInDB(id: number, obj: any) {
    return await this.programCategoryRepository
      .update(obj, { where: { programCategoryId: id } })
      .then((result) => {
        return result;
      })
      .catch((e) => {
