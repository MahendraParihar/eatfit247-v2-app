...
export class CallTypeService {
  ...
  public async findAll(searchDto: BasicSearchDto): Promise<IServerResponse> {
    ...
    try {
      ...
      const { rows, count } = await this.callTypeRepository.findAndCountAll<MstCallType>({
        include: [
          {
            model: MstAdminUser,
            required: false,
  ...
  public async getCallTypeList(): Promise<DropdownListInterface[]> {
  ...
  private async createInDB(obj: any) {
    return await this.callTypeRepository
      .create(obj)
      .then((result) => {
        return result;
      })
  ...
  private async updateInDB(id: number, obj: any) {
    return await this.callTypeRepository
      .update(obj, { where: { callTypeId: id } })
      .then((result) => {
        return result;
      })
      .catch((e) => {
