...
export class PaymentModeService {
  ...
  public async findAll(searchDto: IBasicSearch): Promise<ITableList<ILov>> {
    ...
    if (searchDto.name) {
      whereCondition['paymentMode'] = searchDto.name;
    }
    const pageNumber = searchDto.pageNumber;
    const pageSize = searchDto.pageSize;
    const offset = pageNumber === 0 ? 0 : pageNumber * pageSize;

    const { rows, count } = await this.paymentModeRepository.findAndCountAll<MstPaymentMode>({
      include: [
        {
          model: MstAdminUser,
          required: false,
  ...
  public async fetchById(id: number): Promise<ILov> {
    const find = await this.paymentModeRepository.findOne({
      where: {
        paymentModeId: id,
      },
    });
    ...
    return <ILov>{
      id: find.paymentModeId,
      name: find.paymentMode,
      active: find.active,
  ...
  public async getPaymentModeList(): Promise<IDropdownItem[]> {
    const tempList = await this.paymentModeRepository.findAll<MstPaymentMode>({
      where: {
        active: true,
      },
    });
  ...
  private async createInDB(obj: any) {
    return await this.paymentModeRepository.create(obj);
  }

  private async updateInDB(id: number, obj: any) {
    return await this.paymentModeRepository.update(obj, { where: { paymentModeId: id } });
  }
}
