...
export class PaymentStatusService {
  ...
  public async findAll(searchDto: IBasicSearch): Promise<ITableList<ILov>> {
    ...
    if (searchDto.name) {
      whereCondition['paymentStatus'] = searchDto.name;
    }
    const pageNumber = searchDto.pageNumber;
    const pageSize = searchDto.pageSize;
    const offset = pageNumber === 0 ? 0 : pageNumber * pageSize;

    const { rows, count } = await this.paymentStatusRepository.findAndCountAll<MstPaymentStatus>({
      include: [
        {
          model: MstAdminUser,
          required: false,
  ...
  public async fetchById(id: number): Promise<ILov> {
    const find = await this.paymentStatusRepository.findOne({
      where: {
        paymentStatusId: id,
      },
    });
    ...
    return <ILov>{
      id: find.paymentStatusId,
      name: find.paymentStatus,
      active: find.active,
  ...
  public async getPaymentStatusList(): Promise<IDropdownItem[]> {
    const tempList = await this.paymentStatusRepository.findAll<MstPaymentStatus>({
      where: {
        active: true,
      },
    });
  ...
  private async createInDB(obj: any) {
    return await this.paymentStatusRepository.create(obj);
  }

  private async updateInDB(id: number, obj: any) {
    return await this.paymentStatusRepository.update(obj, { where: { paymentStatusId: id } });
  }
}
