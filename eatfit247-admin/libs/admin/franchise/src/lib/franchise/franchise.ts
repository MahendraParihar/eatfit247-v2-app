...
export class Franchise implements OnInit {
  ...
  totalCount = 0;
  loading = false;
  tableConfig!: TableConfig<IFranchise>;
  private searchSubject = new Subject<string>();

  constructor(private apiService: FranchiseApiService) {
    this.setupSearch();
  }
  ...
  private initializeTable(): void {
    const columns: TableColumn<IFranchise>[] = [
    ...
    const actions: TableAction<IFranchise>[] = [
  ...
  editItem(item: IFranchise): void {
    console.log('Edit franchise:', item);
  }
