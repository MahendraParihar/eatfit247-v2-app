...
export class Referrer implements OnInit {
  ...
  totalCount = 0;
  loading = false;
  tableConfig!: TableConfig<IReferrer>;
  private searchSubject = new Subject<string>();

  constructor(private apiService: ReferrerApiService) {
    this.setupSearch();
  }
  ...
  private initializeTable(): void {
    const columns: TableColumn<IReferrer>[] = [
    ...
    const actions: TableAction<IReferrer>[] = [
  ...
  editItem(item: IReferrer): void {
    console.log('Edit referrer:', item);
  }
