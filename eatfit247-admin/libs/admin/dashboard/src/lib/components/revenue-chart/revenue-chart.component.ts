...
export class RevenueChartComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    try {
      this.data = await this.apiService.getRevenueData();
    } catch {
  ...
  private updateBarChart(): void {
    ...
    const option = {
      ...
      series: [
        {
          name: 'Paid', type: 'bar', stack: 'total',
  ...
  onTabChange(index: number): void {
    ...
    setTimeout(() => {
      if (index === 0 && this.lineChartInstance) this.lineChartInstance.resize();
      else if (index === 1 && this.barChartInstance) this.barChartInstance.resize();
    }, 100);
  }
