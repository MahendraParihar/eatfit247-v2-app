...
import { ITableList, IProduct, IResponse } from '@eatfit247-shared-lib';
...
export class ProductsApiService extends ApiBaseService {
  ...
  async getList(params?: any): Promise<ITableList<IProduct>> {
    const res = await this.httpService.get<IResponse<ITableList<IProduct>>>(`${this.endpoint}/list`, { params });
    return res.data as ITableList<IProduct>;
  }

  async getById(id: number): Promise<IProduct> {
    const res = await this.httpService.get<IResponse<IProduct>>(`${this.endpoint}/manage/${id}`);
