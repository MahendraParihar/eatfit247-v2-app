...
export class LegalPages implements OnInit {
  data: ILegalPageList[] = [];
  totalCount = 0;
  loading = false;
  tableConfig!: ITableConfig<ILegalPageList>;
  ...
  private initializeTable(): void {
    const columns: ITableColumn<ILegalPageList>[] = [
      {
        key: 'legalPageId',
        label: 'ID',
        dataKey: 'legalPageId',
    ...
    const actions: ITableAction<ILegalPageList>[] = [
  ...
  async loadData(): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<ILegalPageList> = await this.apiService.getList({
        page: 0,
  ...
  editItem(item: ILegalPageList): void {
    this.router.navigate(['/legal-pages/edit', item.legalPageId]);
  }

  createItem(): void {
    this.router.navigate(['/legal-pages/new']);
  }

  async toggleStatus(item: ILegalPageList): Promise<void> {
    const action = item.active ? 'deactivate' : 'activate';
    const confirmed = confirm(`Are you sure you want to ${action} "${item.title}"?`);
    if (confirmed) {
      this.loading = true;
      try {
        await this.apiService.updateStatus(item.legalPageId, !item.active);
        await this.loadData();
      } catch {
        this.loading = false;
      }
