...
@Component({
  ...
  templateUrl: './manage-urine-output.html',
  styleUrl: './manage-urine-output.scss'
})
export class ManageUrineOutput implements OnInit {
  private fb: FormBuilder = inject(FormBuilder);
  formGroup: FormGroup = this.fb.group({
    urineOutput: ['', [Validators.required, Validators.minLength(InputLengthEnum.CHAR_2), Validators.maxLength(InputLengthEnum.CHAR_50)]],
    active: [true, [Validators.required]]
  });
  initialData!: IUrineOutput;
  mediaFor = MediaForEnum.URINE_OUTPUT;
  mediaType = FileTypeEnum.IMAGE;
  isEditMode = false;
  pageTitle = 'Create Urine Output';
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (id && id !== 'new') {
      ...
      this.pageTitle = 'Edit Urine Output';
      await this.loadData(+id);
    } else {
      this.pageTitle = 'Create Urine Output';
    }
    this.buildForm();
  }
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.formGroup.valid) {
      const formValue: IManageUrineOutput = { ...this.formGroup.value };
