...
export class BlogAuthor implements OnInit {
  data: any[] = [];
  totalCount = 0;
  loading = false;
  ...
  private setupSearch(): void {
    this.searchSubject.pipe(debounceTime(300), distinctUntilChanged(), switchMap((search) => {
      this.loading = true;
      return this.apiService.getBlogAuthorList({ search, page: 0, limit: this.tableConfig.pageSize || 10 });
    })).subscribe({
  ...
  editItem(item: any): void {
    this.router.navigate(['/lov-master/blog-author/edit', item.blogAuthorId]);
  }

  createItem(): void {
    this.router.navigate(['/lov-master/blog-author/new']);
  }

  async toggleStatus(item: any): Promise<void> {
    const action = item.active ? 'deactivate' : 'activate';
    const confirmed = confirm(`Are you sure you want to ${action} "${item.firstName || item.name}"?`);
    if (confirmed) {
      this.loading = true;
      try {
        await this.apiService.updateBlogAuthorStatus(item.blogAuthorId, !item.active);
        await this.loadData();
      } catch {
        this.loading = false;
      }
