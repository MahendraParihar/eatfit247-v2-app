...
@Component({
  selector: 'lib-manage-eating-habit',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './manage-eating-habit.html',
  styleUrl: './manage-eating-habit.scss'
})
export class ManageEatingHabit implements OnInit {
  private fb: FormBuilder = inject(FormBuilder);
  formGroup: FormGroup = this.fb.group({
    eatingHabit: ['', [Validators.required, Validators.minLength(InputLengthEnum.CHAR_2), Validators.maxLength(InputLengthEnum.CHAR_50)]],
    active: [true, [Validators.required]]
  });
  initialData!: IEatingHabit;
  mediaFor = MediaForEnum.EATING_HABIT;
  mediaType = FileTypeEnum.IMAGE;
  isEditMode = false;
  pageTitle = 'Create Eating Habit';
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (id && id !== 'new') {
      ...
      this.pageTitle = 'Edit Eating Habit';
      await this.loadData(+id);
    } else {
      this.pageTitle = 'Create Eating Habit';
    }
    this.buildForm();
  }
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.formGroup.valid) {
      const formValue: IManageEatingHabit = { ...this.formGroup.value };
