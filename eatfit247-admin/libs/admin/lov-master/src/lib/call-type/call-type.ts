...
export class CallType implements OnInit {
  data: any[] = [];
  totalCount = 0;
  loading = false;
  ...
  private setupSearch(): void {
    this.searchSubject.pipe(debounceTime(300), distinctUntilChanged(), switchMap((search) => {
      this.loading = true;
      return this.apiService.getCallTypeList({ search, page: 0, limit: this.tableConfig.pageSize || 10 });
    })).subscribe({
  ...
  async loadData(): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getCallTypeList({ page: 0, limit: this.tableConfig.pageSize || 10 });
  ...
  async onPageChange(pagination: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getCallTypeList({ page: pagination.pageIndex, limit: pagination.pageSize });
      this.data = response.data;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  async onSortChange(sort: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getCallTypeList({ page: 0, limit: this.tableConfig.pageSize || 10, sortBy: sort.active, sortOrder: sort.direction });
      this.data = response.data;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  editItem(item: any): void {
    this.router.navigate(['/lov-master/call-type/edit', item.callTypeId || item.id]);
  }

  createItem(): void {
    this.router.navigate(['/lov-master/call-type/new']);
  }

  async toggleStatus(item: any): Promise<void> {
    const action = item.active ? 'deactivate' : 'activate';
    const confirmed = confirm(`Are you sure you want to ${action} "${item.callType || item.name}"?`);
    if (confirmed) {
      this.loading = true;
      try {
        await this.apiService.updateCallTypeStatus(item.callTypeId || item.id, !item.active);
        await this.loadData();
      } catch {
        this.loading = false;
      }
