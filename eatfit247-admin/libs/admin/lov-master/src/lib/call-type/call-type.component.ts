...
export class CallType implements OnInit {
  ...
  editItem(item: any): void {
    this.router.navigate(['/lov-master/call-type/edit', item.callTypeId]);
  }
  ...
  async toggleStatus(item: any): Promise<void> {
    ...
    if (confirmed) {
      this.loading = true;
      try {
        await this.apiService.updateCallTypeStatus(item.callTypeId, !item.active);
        await this.loadData();
      } catch {
        this.loading = false;
      }
