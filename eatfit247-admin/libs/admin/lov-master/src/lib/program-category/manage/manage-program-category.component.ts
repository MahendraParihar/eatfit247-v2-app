...
import { FileTypeEnum, IProgramCategory, IManageProgramCategory, InputLengthEnum, MediaForEnum } from '@eatfit247-shared-lib';

@Component({
  selector: 'lib-manage-program-category',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './manage-program-category.html',
  styleUrl: './manage-program-category.scss'
})
export class ManageProgramCategory implements OnInit {
  private fb: FormBuilder = inject(FormBuilder);
  formGroup: FormGroup = this.fb.group({
    programCategory: ['', [Validators.required, Validators.minLength(InputLengthEnum.CHAR_2), Validators.maxLength(InputLengthEnum.CHAR_50)]],
    url: ['', [Validators.maxLength(InputLengthEnum.CHAR_200)]],
    active: [true, [Validators.required]]
  });
  initialData!: IProgramCategory;
  mediaFor = MediaForEnum.PROGRAM_CATEGORY;
  mediaType = FileTypeEnum.IMAGE;
  isEditMode = false;
  pageTitle = 'Create Program Category';
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (id && id !== 'new') {
      ...
      this.pageTitle = 'Edit Program Category';
      await this.loadData(+id);
    } else {
      this.pageTitle = 'Create Program Category';
    }
    this.buildForm();
  }
  ...
  private buildForm(): void {
    this.formGroup = this.fb.group({
      programCategory: [
        this.initialData?.programCategory || '',
        [
          Validators.required,
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.formGroup.valid) {
      const formValue: IManageProgramCategory = { ...this.formGroup.value };
