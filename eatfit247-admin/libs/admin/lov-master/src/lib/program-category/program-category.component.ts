...
export class ProgramCategory implements OnInit {
  ...
  private setupSearch(): void {
    this.searchSubject.pipe(debounceTime(300), distinctUntilChanged(), switchMap((search) => {
      this.loading = true;
      return this.apiService.getProgramCategoryList({ search, page: 0, limit: this.tableConfig.pageSize || 10 });
    })).subscribe({
  ...
  async loadData(): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getProgramCategoryList({ page: 0, limit: this.tableConfig.pageSize || 10, search: this.currentSearch?.trim() || undefined });
  ...
  async onPageChange(pagination: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getProgramCategoryList({ page: pagination.pageIndex, limit: pagination.pageSize, search: this.currentSearch?.trim() || undefined });
      this.data = response.tableData;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  async onSortChange(sort: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getProgramCategoryList({ page: 0, limit: this.tableConfig.pageSize || 10, sortBy: sort.active, sortOrder: sort.direction, search: this.currentSearch?.trim() || undefined });
      this.data = response.tableData;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  editItem(item: any): void {
    this.router.navigate(['/lov-master/program-category/edit', item.programCategoryId]);
  }

  createItem(): void {
    this.router.navigate(['/lov-master/program-category/new']);
  }

  async toggleStatus(item: any): Promise<void> {
    const action = item.active ? 'deactivate' : 'activate';
    const confirmed = confirm(`Are you sure you want to ${action} "${item.programCategory || item.name}"?`);
    if (confirmed) {
      this.loading = true;
      try {
        await this.apiService.updateProgramCategoryStatus(item.programCategoryId, !item.active);
        await this.loadData();
      } catch {
        this.loading = false;
      }
