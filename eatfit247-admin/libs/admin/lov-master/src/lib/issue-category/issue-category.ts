...
export class IssueCategory implements OnInit {
  data: any[] = [];
  totalCount = 0;
  loading = false;
  ...
  private setupSearch(): void {
    this.searchSubject.pipe(debounceTime(300), distinctUntilChanged(), switchMap((search) => {
      this.loading = true;
      return this.apiService.getIssueCategoryList({ search, page: 0, limit: this.tableConfig.pageSize || 10 });
    })).subscribe({
  ...
  async toggleStatus(item: any): Promise<void> {
    ...
    const confirmed = confirm(`Are you sure you want to ${action} "${item.issueCategory || item.name}"?`);
    if (confirmed) {
      this.loading = true;
      try {
        await this.apiService.updateIssueCategoryStatus(item.issueCategoryId || item.id, !item.active);
        await this.loadData();
      } catch {
        this.loading = false;
      }
