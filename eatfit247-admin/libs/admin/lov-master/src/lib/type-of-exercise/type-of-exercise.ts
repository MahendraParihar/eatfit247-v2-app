...
export class TypeOfExercise implements OnInit {
  ...
  totalCount = 0;
  loading = false;
  tableConfig!: TableConfig<ITypeOfExercise>;
  ...
  private initializeTable(): void {
    const columns: TableColumn<ITypeOfExercise>[] = [
    ...
    const actions: TableAction<ITypeOfExercise>[] = [
  ...
  async loadData(): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<ITypeOfExercise> = await this.apiService.getTypeOfExerciseList({ page: 0, limit: this.tableConfig.pageSize || 10 });
  ...
  async onPageChange(pagination: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<ITypeOfExercise> = await this.apiService.getTypeOfExerciseList({ page: pagination.pageIndex, limit: pagination.pageSize });
      this.data = response.data;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  async onSortChange(sort: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<ITypeOfExercise> = await this.apiService.getTypeOfExerciseList({ page: 0, limit: this.tableConfig.pageSize || 10, sortBy: sort.active, sortOrder: sort.direction });
      this.data = response.data;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  editItem(item: ITypeOfExercise): void {
    console.log('Edit type of exercise:', item);
  }
