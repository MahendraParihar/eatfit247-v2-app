...
import { FileTypeEnum, ITypeOfExercise, IManageTypeOfExercise, InputLengthEnum, MediaForEnum } from '@eatfit247-shared-lib';

@Component({
  selector: 'lib-manage-type-of-exercise',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './manage-type-of-exercise.html',
  styleUrl: './manage-type-of-exercise.scss'
})
export class ManageTypeOfExercise implements OnInit {
  private fb: FormBuilder = inject(FormBuilder);
  formGroup: FormGroup = this.fb.group({
    typeOfExercise: ['', [Validators.required, Validators.minLength(InputLengthEnum.CHAR_2), Validators.maxLength(InputLengthEnum.CHAR_50)]],
    active: [true, [Validators.required]]
  });
  initialData!: ITypeOfExercise;
  mediaFor = MediaForEnum.TYPE_OF_EXERCISE;
  mediaType = FileTypeEnum.IMAGE;
  isEditMode = false;
  pageTitle = 'Create Type of Exercise';
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (id && id !== 'new') {
      ...
      this.pageTitle = 'Edit Type of Exercise';
      await this.loadData(+id);
    } else {
      this.pageTitle = 'Create Type of Exercise';
    }
    this.buildForm();
  }
  ...
  private buildForm(): void {
    this.formGroup = this.fb.group({
      typeOfExercise: [
        this.initialData?.typeOfExercise || '',
        [
          Validators.required,
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.formGroup.valid) {
      const formValue: IManageTypeOfExercise = { ...this.formGroup.value };
