...
export class State implements OnInit {
  ...
  private initializeTable(): void {
    const columns: TableColumn<any>[] = [
      ...
      { key: 'country', label: 'Country', dataKey: 'country', sortable: false, formatter: (value) => value?.country || '-' },
  ...
  async loadData(): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getStateList({ page: 0, limit: this.tableConfig.pageSize || 10 });
  ...
  async onPageChange(pagination: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getStateList({ page: pagination.pageIndex, limit: pagination.pageSize });
      this.data = response.data;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  async onSortChange(sort: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getStateList({ page: 0, limit: this.tableConfig.pageSize || 10, sortBy: sort.active, sortOrder: sort.direction });
      this.data = response.data;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  async toggleStatus(item: any): Promise<void> {
    ...
    if (confirmed) {
      this.loading = true;
      try {
        await this.apiService.updateStateStatus(item.stateId || item.id, !item.active);
        await this.loadData();
      } catch {
        this.loading = false;
      }
