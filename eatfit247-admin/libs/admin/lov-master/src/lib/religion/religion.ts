...
export class Religion implements OnInit {
  ...
  totalCount = 0;
  loading = false;
  tableConfig!: TableConfig<IReligion>;
  ...
  private initializeTable(): void {
    const columns: TableColumn<IReligion>[] = [
    ...
    const actions: TableAction<IReligion>[] = [
  ...
  async loadData(): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<IReligion> = await this.apiService.getReligionList({ page: 0, limit: this.tableConfig.pageSize || 10 });
  ...
  async onPageChange(pagination: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<IReligion> = await this.apiService.getReligionList({ page: pagination.pageIndex, limit: pagination.pageSize });
      this.data = response.data;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  async onSortChange(sort: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<IReligion> = await this.apiService.getReligionList({ page: 0, limit: this.tableConfig.pageSize || 10, sortBy: sort.active, sortOrder: sort.direction });
      this.data = response.data;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
