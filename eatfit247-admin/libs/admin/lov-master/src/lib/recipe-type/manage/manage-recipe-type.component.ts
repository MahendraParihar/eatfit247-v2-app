...
import { FileTypeEnum, IRecipeType, IManageRecipeType, InputLengthEnum, MediaForEnum } from '@eatfit247-shared-lib';

@Component({
  selector: 'lib-manage-recipe-type',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './manage-recipe-type.html',
  styleUrl: './manage-recipe-type.scss'
})
export class ManageRecipeType implements OnInit {
  private fb: FormBuilder = inject(FormBuilder);
  formGroup: FormGroup = this.fb.group({
    recipeType: ['', [Validators.required, Validators.minLength(InputLengthEnum.CHAR_2), Validators.maxLength(InputLengthEnum.CHAR_50)]],
    active: [true, [Validators.required]]
  });
  initialData!: IRecipeType;
  mediaFor = MediaForEnum.RECIPE_TYPE;
  mediaType = FileTypeEnum.IMAGE;
  isEditMode = false;
  pageTitle = 'Create Recipe Type';
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (id && id !== 'new') {
      ...
      this.pageTitle = 'Edit Recipe Type';
      await this.loadData(+id);
    } else {
      this.pageTitle = 'Create Recipe Type';
    }
    this.buildForm();
  }
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.formGroup.valid) {
      const formValue: IManageRecipeType = { ...this.formGroup.value };
