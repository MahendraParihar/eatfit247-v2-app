...
@Component({
  selector: 'lib-manage-health-issue',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './manage-health-issue.html',
...
export class ManageHealthIssue implements OnInit {
  ...
  initialData!: IHealthIssue;
  mediaFor = MediaForEnum.HEALTH_ISSUE;
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (id && id !== 'new') {
      ...
      this.pageTitle = 'Edit Health Issue';
      await this.loadData(+id);
    } else {
      this.pageTitle = 'Create Health Issue';
    }
    this.buildForm();
  }
  ...
  async onSubmit(): Promise<void> {
    ...
    if (this.formGroup.valid) {
      const formValue: IManageHealthIssue = { ...this.formGroup.value };
