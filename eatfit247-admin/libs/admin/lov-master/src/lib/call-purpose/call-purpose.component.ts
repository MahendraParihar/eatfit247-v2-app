...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, DataTableComponent, RouterModule, MatButtonModule, MatIconModule],
  templateUrl: './call-purpose.html',
  styleUrl: './call-purpose.scss',
})
export class CallPurpose implements OnInit {
  data: any[] = [];
  totalCount = 0;
  loading = false;
  ...
  private setupSearch(): void {
    this.searchSubject.pipe(debounceTime(300), distinctUntilChanged(), switchMap((search) => {
      this.loading = true;
      return this.apiService.getCallPurposeList({ search, page: 0, limit: this.tableConfig.pageSize || 10 });
    })).subscribe({
  ...
  async loadData(): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getCallPurposeList({ page: 0, limit: this.tableConfig.pageSize || 10, search: this.currentSearch?.trim() || undefined });
  ...
  async onPageChange(pagination: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getCallPurposeList({ page: pagination.pageIndex, limit: pagination.pageSize, search: this.currentSearch?.trim() || undefined });
      this.data = response.tableData;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  async onSortChange(sort: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getCallPurposeList({ page: 0, limit: this.tableConfig.pageSize || 10, sortBy: sort.active, sortOrder: sort.direction, search: this.currentSearch?.trim() || undefined });
      this.data = response.tableData;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  editItem(item: any): void {
    this.router.navigate(['/lov-master/call-purpose/edit', item.callPurposeId || item.id]);
  }

  createItem(): void {
    this.router.navigate(['/lov-master/call-purpose/new']);
  }

  async toggleStatus(item: any): Promise<void> {
    const action = item.active ? 'deactivate' : 'activate';
    const confirmed = confirm(`Are you sure you want to ${action} "${item.callPurpose || item.name}"?`);
    if (confirmed) {
      this.loading = true;
      try {
        await this.apiService.updateCallPurposeStatus(item.callPurposeId || item.id, !item.active);
        await this.loadData();
      } catch {
        this.loading = false;
      }
