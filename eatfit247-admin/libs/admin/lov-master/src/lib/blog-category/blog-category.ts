...
export class BlogCategory implements OnInit {
  ...
  async loadData(): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getBlogCategoryList({ page: 0, limit: this.tableConfig.pageSize || 10 });
  ...
  async onPageChange(pagination: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getBlogCategoryList({ page: pagination.pageIndex, limit: pagination.pageSize });
      this.data = response.data;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  async onSortChange(sort: any): Promise<void> {
    this.loading = true;
    try {
      const response: ITableList<any> = await this.apiService.getBlogCategoryList({ page: 0, limit: this.tableConfig.pageSize || 10, sortBy: sort.active, sortOrder: sort.direction });
      this.data = response.data;
      this.totalCount = response.count;
      this.loading = false;
    } catch {
  ...
  editItem(item: any): void {
    console.log('Edit blog category:', item);
  }
  ...
  async toggleStatus(item: any): Promise<void> {
    ...
    if (confirmed) {
      this.loading = true;
      try {
        await this.apiService.updateBlogCategoryStatus(item.blogCategoryId || item.id, !item.active);
        await this.loadData();
      } catch {
        this.loading = false;
      }
