...
export class MediaPress implements OnInit {
  ...
  totalCount = 0;
  loading = false;
  tableConfig!: TableConfig<IPressMedia>;
  private searchSubject = new Subject<string>();

  constructor(private apiService: PressMediaApiService) {
    this.setupSearch();
  }
  ...
  private initializeTable(): void {
    const columns: TableColumn<IPressMedia>[] = [
    ...
    const actions: TableAction<IPressMedia>[] = [
  ...
  editItem(item: IPressMedia): void {
    console.log('Edit media & press:', item);
  }
